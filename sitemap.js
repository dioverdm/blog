...
function main() {
  ...
  const blogEntries = getFilesRecursively(blogPostsDir, '_', ['.mdx', '.tsx']).map((file) => ({
  ...
  const excludedEntries = allEntries.filter(entry => {
    return shouldExclude = !(entry.url.replace(/\\/g, '/') === '/blog/index' || entry.url.replace(/\\/g, '/') === '/index' || entry.url.includes('[slug]'));
  });
